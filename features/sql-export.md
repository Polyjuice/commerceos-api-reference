...
| Streaming | Batched output with configurable `batchSize` (default 1000) |
| Array/Object Handling | **Rejected** — must be flattened or JSON-stringified before serialization |
| Mode Parameter | **Hint only** — `mode=insert|sync|merge` is passed to mapped types; serializer emits statements as-is |
...
> **Note:** The `mode` parameter (`insert`, `sync`, `merge`) is a **hint to mapped types** that produce the `SqlStatement[]` array. The SQL serializer itself emits statements exactly as provided—it does not interpret or transform based on mode.
...
> **Note:** With the removal of `@merge`, each source item produces its own INSERT statement. For batched inserts with multiple VALUES rows, aggregate using array-body mapped types with `$prior` and `"$first"`.
...
SKU-001,Widget,29.9900,SEK,Active
SKU-002,Gadget,49.9900,SEK,Active
```
...
The SQL serializer **rejects** arrays and objects. Before serialization, you must:
...
|-----------|---------------|
| Unknown statement type | `SQL serialization error: Unknown statement type` |
| Empty WHERE clause | `SQL serialization error: DELETE statement requires at least one WHERE condition` |
| Empty VALUES array | `SQL serialization error: INSERT statement requires at least one row` |
| Non-finite number | `SQL serialization error: Cannot serialize Infinity (non-finite number)` |
| Array value | `SQL serialization error: Arrays must be serialized as JSON columns or separate tables` |
| Object value | `SQL serialization error: Objects must be flattened or serialized as JSON columns` |
