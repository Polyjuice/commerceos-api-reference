...
5. Execute import in dependency order:
   - Roles/permissions before users.
   - Devices before POS terminals.
   - Payment methods before payment terminals/integrations.
   - Config API singletons early (or before dependent resources).
6. For each resource:
...
| `/v1/config/ms-graph-email` | `clientSecret` |
...
- In import preflight, warn users when OAuth2 clients are required before EPI `install` can run; include a blocking prompt or checklist for that step.
