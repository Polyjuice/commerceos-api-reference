...
| `currency` | No | Defaults to order currency |
...
| Date range | `/receipts/after/2024-12-01T00:00:00Z/before/2024-12-31T23:59:59Z` |
| With item count | `/receipts~with(items/count)` |

**Performance Tip:** Use `/receipts/after/` and `/receipts/before/` - these use database indexes. Avoid `~where(timestamp>...)` which filters in-memory.
...
**Default responses include only essential fields:**
...
| `~map(type)` | Transform | `~map(com.heads.receipt-csv)` | Requires mapped type to exist |
| `~flat` | Flatten arrays | `~map(items)~flat` | NO parentheses |
| `~distinct` | Unique values | `~map(status)~distinct` | NO parentheses, primitives only |
...
### 7.2 Streaming JSON Responses

For fast first-byte JSON responses, use the `stream=true` parameter in the Accept header:

```bash
# Streaming JSON - fast first byte (recommended for large result sets)
curl -u ":banana" -H "Accept: application/json; stream=true" \
...
|---------------|----------|------------|----------|
| `application/json` | Buffered - waits for all items | Delayed | Small datasets, simple parsing |
| `application/json; stream=true` | Streaming - emits items as available | Fast | Large datasets, progress feedback |
| `application/x-ndjson` | Line-delimited - one object per line | Fast | CLI pipelines, memory-constrained |
...
| `application/json; stream=true` | Full | Streaming | All items read |
...
2. **Endpoint exists?**
   ```bash
   curl -u ":banana" "localhost:5000/api/v1/products/properties"
   ```
