...
| `reference/resource-patterns.md` | Type member reference |
...
|----------|-------------|---------|
| ~where() | Filter | ~/products~where(status=Active)` |
| ~with() | Expand fields | `~with(prices)` |
...
- Agent references need `@type` and `identifiers`
...
## Examples
GET /products~where(status=Active)~take(10)
GET /trade-orders~with(items,customer)~first
```
