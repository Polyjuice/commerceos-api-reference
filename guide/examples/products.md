...
> **Important:** All entities in the hierarchy (`product group`, `product family`, `product`) are considered **product nodes** and share common fields like `identifiers`, `name`, `images`, `prices`, `categories`, `labels`.
...
Groups can define `variantDimensions` that families inherit:
...
VAT codes cascade from groups to products:
...
# Products without their own VAT code inherit from parentGroup
```
...
Sales channels control where products are available (POS, webshop, mobile). These are inherited fields on `product/family`/`product node`.
