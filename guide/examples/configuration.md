...
# NOTE: X-Request-Map selectors must match raw JSON keys (not type-based member paths).
# The mapped type body should use literal keys that correspond to incoming JSON fields:
#   {
...
> **Important: X-Request-Map Limitation**
>
> When using `X-Request-Map`, the selectors in the mapped type body must reference raw JSON keys
> from the incoming request, not schema-typed member paths. For example, if your incoming JSON has
> `{"sku": "P-001", "title": "My Product"}`, the mapping body should use `"sku"` and `"title"`,
> not path expressions like `identifiers/com.myapp.sku`. Type-based paths won't resolve correctly
> because the request body is raw JSON without Pillow type context.
