...
Stock reset is a method endpoint (`POST /v1/stock-reset`) that clears all stock for an owner. It accepts an `owner` (required) and optionally a `product` to limit the reset scope.

```bash
# Reset all stock for an owner (clears all products at all stock places)
...
>
> **Note:** Stock reset does NOT accept `stockPlace` or `quantity` parameters. It operates at the owner level, resetting physical stock quantities to match available quantities across all the owner's stock places.
