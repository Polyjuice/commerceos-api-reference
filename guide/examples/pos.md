...
# Optional: status, assignedDevice, associatedNode, receiptPrinter, etc.
...
# Required fields: identifiers.receiptID, prefix, ordinal, seller, buyer, posTerminal, items, payments
...
# Required: method (payment method) and directUrl (terminal communication URL)
# Optional: name, connectedDevice
# Note: There is NO `associatedNode` field on payment terminals
curl -X POST -u ":banana" "localhost:5000/api/v1/payment-terminals" \
  ...
  -d '{
    ...
    "directUrl": "https://payment-gateway.example.com/terminals/TERM-001"
  }'
