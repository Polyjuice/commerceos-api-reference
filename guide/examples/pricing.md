...
> **Note:** Currencies only expose `identifiers.currencyCode` — no `name`, `symbol`, or `decimalDigits` fields.
...
# Create/update currency (identifiers.currencyCode only)
...
- **Currencies**: Only `identifiers.currencyCode` is supported — `name`, `symbol`, `decimalDigits` fields do NOT exist
