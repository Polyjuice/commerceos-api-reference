...
Two rules implement tiered pricing: 5% off at 3+ items, 10% off at 5+ items. Both rules use the same phase priority so they are evaluated together. Since they have non-overlapping quantity conditions (`atLeast: 3` vs `atLeast: 5`), only the applicable tier will match and apply—the `atLeast` constraint naturally prevents multiple tiers from stacking:

> **Note:** Phase priority determines the *order* in which phases are evaluated, not which rules "win." Rules within the same phase can stack if their conditions overlap. Use distinct `atLeast`/`atMost` constraints to create mutually exclusive tiers.
