...
10. GET /people~where(!email)~take(10)
```
...
49. GET /companies~with(assortmentRoots~just(name, status))~take(5)
50. GET /companies~with(assortment~where(status=Active)~take(5))
```
...
90.  GET /products~just(name, plu, gtin)~where(plu)
```
...
118. GET /products~with(images~just(url, mimeType))~take(10)
...
144. GET /products~with(stockLevels~with(location~just(name, owner/name)))~take(3)
...
### Hierarchy
```
151. GET /product-categories~with(parent)~take(20)
152. GET /product-categories~just(name, parentName:parent/name)
153. GET /product-categories~with(childCategories)~take(10)
154. GET /product-categories~just(name, childCount:childCategories~count)
155. GET /product-categories~where(!parent)~take(20)
```
...
### Complex
```
161. GET /product-categories~where(!parent)~with(childCategories~just(name))
162. GET /product-categories~just(name, parent/name, childCategories~count)~take(50)
...
165. GET /product-categories~with(parent~just(name), childCategories~just(name)~take(5))~take(10)
```

### Deep Hierarchy
```
166. GET /product-categories~where(!parent)~with(childCategories~with(childCategories~just(name)))~take(5)
...
169. GET /product-categories~where(!parent)~orderBy(name)~with(childCategories~orderBy(name))
...
### Hierarchy
```
176. GET /product-groups~with(parent)~take(20)
177. GET /product-groups~just(name, parentName:parent/name)
178. GET /product-groups~with(children)~take(10)
179. GET /product-groups~just(name, childCount:children~count)
180. GET /product-groups~where(!parent)~take(20)
```
...
### Complex
```
186. GET /product-groups~where(!parent)~with(children~just(name))
187. GET /product-groups~just(name, parent/name, children~count)~take(50)
188. GET /product-groups~with(children~with(members~take(2)))~take(3)
...
255. GET /trade-orders~with(payments~just(amount, method), shipments~just(status))~take(5)
```
...
284. GET /prices~with(sellers~with(owner~just(name)))~take(5)
...
Note: `/stock-transactions` may not exist as top-level. Use `/stock-places` with `transactions` member.
```
