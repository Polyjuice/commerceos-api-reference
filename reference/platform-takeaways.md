...
The `$prior` selector references data from the source context, and `"$first"` trims the array to a single object.

### Request Mapping (Experimental)

The `X-Request-Map` header is parsed but may fail on raw JSON bodies because selectors are resolved without type context. Use with caution or consider post-processing writes instead.
