...
GET /api/v1/products~with(prices,categories)

# Deep nesting - expand items, then product on each item, then prices on product
GET /api/v1/trade-orders~with(items~with(product~with(prices)))

# Whitelist: only return name and status
GET /api/v1/products~just(name,status)

# Blacklist: exclude audit fields
GET /api/v1/products~without(createdAt,updatedAt,createdBy)
```
