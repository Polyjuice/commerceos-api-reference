...
POST /v1/companies
{
  ...
  "name": "Acme Sweden AB",
  "parent": {"identifiers": {"com.example.companyId": "CORP-001"}}
}
...
POST /v1/stores
{
  ...
  "name": "Downtown Store",
  "owner": {"identifiers": {"com.example.companyId": "COMP-001"}},
  "addresses": {
    "main": {
...
| Relationship | Used On | Field |
|--------------|---------|-------|
| `owner` | Stores | Links store to owning company |
| `parent` | Companies | Links subsidiary to parent company |
...
```bash
# Create the group with an owner
...
| Find by email/phone | POST | `/v1/agents/@find` | Heuristic lookup |
...
1. **Agent finder uses POST (results in response body):**
   ```bash
   # WRONG
...
| GET for agent finder | 405 Method Not Allowed | Use POST |
