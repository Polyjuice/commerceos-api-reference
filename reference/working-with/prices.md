...
| `to` | datetime | Valid until date/time (inclusive) |
...
- Still requires `currency` to be specified
...
> **Note:** The currency resource only exposes `identifiers.currencyCode`. Fields like `name`, `symbol`, and `decimalDigits` are not available on the currency schema, and there is no `denominations` member.
