...
# Assign with weight (for sorting)
POST /v1/products/com.example.sku=PROD-001/categories
{
...
| Delete product | DELETE | `/v1/products/{id}` | **Hard delete** (permanently removes) |
| Soft delete | PATCH | `/v1/products/{id}` | Use `{"status": "Inactive"}` instead |
...
- Products referenced in orders cannot be deleted
- Status changes to `Inactive` preserve historical data
