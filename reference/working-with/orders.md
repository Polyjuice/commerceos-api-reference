...
10. [Order Actions](#order-actions)
11. [Payments](#payments)
12. [Shipments](#shipments)
13. [Order Addresses](#order-addresses)
14. [Returns and Refunds](#returns-and-refunds)
...
18. [Integration Playbook](#integration-playbook)
19. [Case Study: Mobile Device Bundle Sale](#case-study-mobile-device-bundle-sale)
...
|--------|-------------|----------------|
| `New` | Just created, no reservations | Reserved, Cancelled |
| `Reserved` | Stock reserved for the order | Committed, Unreserved, Cancelled |
| `Unreserved` | Reservation released | Reserved, Cancelled |
...
| `quantity` | integer | Conditional | Quantity ordered (use if not using instances) |
...
Order items are created with the order and are **read-only afterward**. To change quantities or products, cancel and recreate the order with updated items.
...
|----------------|-------------|----------|
| `Goods` | Physical products | Phones, accessories, hardware |
| `Services` | Non-physical products | Plans, subscriptions, installation |
...
| `SerializedProduct` | `serialNumber` | General serial number |
...
3. **Instance count = quantity:** When using instances, the quantity is derived from the instance array length.
...
### Payment Orders are Read-Only

Payment orders (`/v1/payment-orders`) are **read-only resources**. You cannot create them directly via `POST /v1/payment-orders`. Instead, use the `createPayment` action on trade orders to record payments.
...
### Shipment Orders are Read-Only

Shipment orders (`/v1/shipment-orders`) are **read-only resources**. You cannot create them directly via `POST /v1/shipment-orders`. Instead, use the `createShipment` action on trade orders:
...
> **Note:** Payment orders are read-only. Use the `createPayment` action on trade orders to record payments.

### Shipment Orders (Read-Only)
...
> **Note:** Shipment orders are read-only. Use the `createShipment` action on trade orders to create shipments. The only available action on shipment orders is `release`.
