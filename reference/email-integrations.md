...
- `@commerceos/connector-smtp` contains SMTP send logic and self-registers as a provider on import.
- `@commerceos/connector-msgraph` contains MS Graph send logic and self-registers as a provider on import.
- `@commerceos/email` registers the **only** SendEmailTask handler and orchestrates provider selection via `selectActiveProvider()`.
