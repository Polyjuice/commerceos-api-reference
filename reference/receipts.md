...
| `ordinal` | number | Sequential number within prefix |
...
| `payments` | array | At least one payment (method, amount required; printouts optional) |
...
# Or use the finder with timestampFrom
POST /v1/receipts/@find
{"timestampFrom": "2025-02-13T09:20:32.710Z"}
```
...
# Access a specific item's discounts
GET /v1/receipts/{id}/items/0/discounts
```
...
```bash
# Get all instances for a receipt item
GET /v1/receipts/{id}/items/0/instances

# Get a specific instance
GET /v1/receipts/{id}/items/0/instances/0
```
...
The finder allows property-based searching:

```bash
POST /v1/receipts/@find
{
  "timestampFrom": "2025-02-13T00:00:00.000Z"
}

# Response includes a "results" collection of receipts
```

The finder accepts:
- `timestampFrom`: ISO 8601 date-time for minimum timestamp
- Additional property filters as key-value pairs
