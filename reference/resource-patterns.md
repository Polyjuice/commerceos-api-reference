...
> **Note:** These endpoints use JavaScript's `new Date(timestamp)` parsing internally. ISO 8601 timestamps are recommended for consistency. Invalid timestamps return an empty array (no error). Relative syntax and combined ranges are not supported—use operators for more complex filtering:
...
Use `/receipts/after/` and `/receipts/before/` for efficient date queries (uses database indexes). These endpoints accept ISO 8601 timestamps only:
...
> **Note:** Invalid timestamps return an empty array (no error). For relative time queries, compute the ISO timestamp in your application code.
...
> **Important:** `associatedNode` must reference an **agent** (store or company), not a product node. Terminal creation will fail if given a product reference.
