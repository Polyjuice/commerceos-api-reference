...
salesChannels, designatedStockPlaces, stockLevels, receiptText, signText
```
...
vatPercentage, package, splitFrom, shipmentItems, instances, manualDiscount, discountable
```
...
seller, buyer, relationship, user, device, posTerminal, items, labels,
totalAmount (read-only), totalTaxAmount (read-only), totalDiscountAmount (read-only),
roundingAmount (read-only), totalPayableAmount (read-only), totalPaidAmount (read-only),
totalExternalSettlementsAmount (read-only), vatGroups (read-only), payments (read-only)
```
