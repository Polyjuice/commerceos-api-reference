...
- With `$first`: mapping a collection returns only the first mapped result (not an array)
...
- Use `"$first"` in an array-body to return a single result instead of an array (see [Array Body](#array-body-pipeline) above).
...
1. Uses `$prior` to reference the receipt being mapped
2. Returns a single result (due to `"$first"`) instead of an array
...
The `"$first"` sentinel ensures the mapping returns a single bundle object when mapping a collection.
