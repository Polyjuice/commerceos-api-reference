...
| `instanceProperties` | object | Variant attributes (size, color) |
...
Currencies are identified by their ISO 4217 currency code. The only supported identifier field is `currencyCode`.
...
> **Note:** Currencies accept only `identifiers.currencyCode`. Other fields like `name`, `symbol`, and `decimalDigits` are not persisted.
...
Category assignments require a `weight` field for ordering within the category.

```bash
# Assign phone to categories (weight controls display order)
...
> **Note:** The `weight` field is required for category assignments. Higher weights sort later. Use this to control product ordering within categories.
