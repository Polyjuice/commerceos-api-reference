...
| `customerGroups` | array | Group memberships for pricing |
...
| `customerGroups` | (from CRM) | (local assigns) | Merge both |
...
function resolveConflict(crmRecord, commerceOsRecord) {
  const merged = {
    ...
    // Merge customer groups (union)
    customerGroups: [
      ...new Set([
        ...commerceOsRecord.customerGroups,
        ...crmRecord.customerGroups
      ])
    ]
  };
...
{
  "identifiers": {"com.acme.mapped-type": "crm-customer-export"},
...
PUT /v1/sync-webhooks/com.acme.sync-id=customer-export
{
  ...
  "maxAttempts": 5,
  "in": {
    "url": "api/v1/people~with(addresses,contactMethods,customerGroups,labels)~take(100)"
  },
  "map": {
    "identifiers": {"key": "com.acme.mapped-type=crm-customer-export"}
  },
  "out": {
    "method": "POST",
...
> **Important**: The `map` field must reference an existing mapped type by key. Inline `map.body` definitions in webhooks are ignored. The mapped type must have `active: true` to be applied.
