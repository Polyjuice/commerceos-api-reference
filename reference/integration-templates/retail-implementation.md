...
PUT /products/com.northwind-retail.bc-id=SKU-001
{
  "name": "Wireless Headphones",
...
PUT /payment-terminals
[
  {
    "identifiers": { "com.northwind-retail.terminal-id": "TERM-001" },
    "name": "Card Terminal 1",
    "method": { "identifiers": { "methodId": "com.heads.card" } },
    "directUrl": "https://terminal-001.local:8443/api"
  }
]
```

> **Note:** Payment terminals require `method` (the payment method reference) and `directUrl` (the terminal's direct API endpoint). The field `associatedNode` is not supported on payment terminals.
...
GET /receipts~orderBy(timestamp:desc)~take(100)
```
...
```json
{
  ...
  "repeat": true,
  "in": {
    "url": "api/v1/receipts~orderBy(timestamp:desc)~take(50)"
  },
  "out": {
    "method": "POST",
...
GET /stock-adjustments~orderBy(timestamp:desc)~take(100)
```
...
```json
{
  ...
  "repeat": true,
  "in": {
    "url": "api/v1/stock-adjustments~orderBy(timestamp:desc)~take(50)"
  },
  "out": {
    "method": "POST",
...
PUT /products/com.test-integrator.bc-id=SKU-001
{
  "name": "Phone",
  "identifiers": {
...
GET /products/com.test-integrator.bc-id=SKU-001
```
...
- **Stock adjustments:** `identifiers`, `items` (each item needs `product`, `place`, `reason`; `quantity` defaults to 1)
