...
Receipt
  ...
  │     ├── quantity ─────────────── Units sold
  │     ├── unitAmount ───────────── Price per unit (incl. VAT)
...
- [ ] `items[].unitAmount` — Unit price (incl. VAT)
...
- [ ] `items[].instances` — IMEI/serial data
...
|-------------|-------------|-------------|-------------|----------|------------|------------|----------|----------|
| abc123 | 1 | PROD-001 | Widget | 2 | 500.00 | 1000.00 | 0.00 | 25 |
| abc123 | 2 | PROD-002 | Gadget | 1 | 250.00 | 250.00 | 0.00 | 25 |
...
PUT /v1/mapped-types/com.acme.mapped-type=receipt-bi-export
{
  "identifiers": {"com.acme.mapped-type": "receipt-bi-export"},
  "active": true,
  "body": {
...
GET /v1/receipts/after/2024-12-15T00:00:00Z~map(com.acme.receipt-bi-export)~take(500)
```
...
GET /v1/receipts/{receipt-key}~with(items~with(product,discounts))
...
GET /v1/spot-exchange-rates~where(targetCurrency/identifiers/currencyCode=SEK)~take(100)
```
...
GET /v1/receipts/{id}~with(items~with(product,discounts),payments,vatGroups)
