...
**Note:** Stock reservation is controlled via the `reservedUntil` field on orders or items. Setting `reservedUntil` reserves stock until the specified time; it does not change the order status to "Reserved". Fulfillment happens via shipment orders, which are separate resources.
...
### Payment Orders (Read-Only Resource)

Payment orders are **read-only** resources created automatically by payment processing. You cannot create them directly via POST. Instead, use the `createPayment` action on trade orders:

```bash
# Record payment via order action (recommended)
PATCH /v1/trade-orders/com.acme.order-id=WEB-2024-123456/actions
{
  "createPayment": {
    "transactionId": "STRIPE-PI-abc123xyz",
...
**Note:** Stock reservation is controlled via `reservedUntil`. Setting `reservedUntil` triggers a reservation and changes status to `Reserved`. You can approve directly from either `New` or `Reserved` status.
...
### Shipment Orders (Read-Only Resource)

Shipment orders are **read-only** resources created automatically via trade order actions. You cannot create them directly via POST.

Create shipments using the trade order `createShipment` action:
...
The shipment order is created with:
...
{
  "identifiers": {"com.acme.mapped-type": "order-status-map"},
  "active": true,
  "body": {
...
# Step 2: Create the sync webhook referencing the mapped type
...
> **Important:** Sync webhooks require referencing an existing **active** mapped type by key. Inline `map.body` payloads are ignored.
...
|-----------------|----------------|------------------|
| pending | processing | Preparing shipment |
| in transit | shipped | Order shipped |
...
{
  "identifiers": {"com.acme.mapped-type": "shipment-completed-map"},
  "active": true,
  "body": {
...
# Step 2: Create the sync webhook
...
> **Note:** Shipment orders are read-only resources. Use `createShipment` action on the trade order to create shipments, then `release` action on the shipment to mark it as shipped.
