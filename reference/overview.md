...
|-------|-------------|-------|----------|
| Organization | People, companies, and stores used across orders and inventory | [Customers guide](working-with/customers.md) | [Examples](../guide/examples.md#organization-examples) |
| Products | Catalog items, categories, groups/families, and pricing metadata | [Products guide](working-with/products.md) | [Examples](../guide/examples.md#products-examples) |
| Pricing | Price rules, validity periods, and currency handling | [Prices guide](working-with/prices.md), [VAT guide](working-with/vat.md) | [Examples](../guide/examples.md#pricing-examples) |
| Orders | Trade orders, items, payments, and returns | [Orders guide](working-with/orders.md) | [Examples](../guide/examples.md#orders-examples) |
| Inventory | Stock places, stock transactions, and adjustment reasons | [Stock guide](working-with/stock.md) | [Examples](../guide/examples.md#inventory-examples) |
| POS | Terminals, profiles, receipts, and payment methods | [Receipts](receipts.md) | [Examples](../guide/examples.md#pos-examples) |
| Users | User accounts, credentials, and role assignments | — | [Examples](../guide/examples.md#users-examples) |
| Configuration | System settings and serial number sequences | — | [Examples](../guide/examples.md#configuration-examples) |
| Advanced | Mapped types and sync webhooks (requires `advanced` scope) | [Sync Webhooks](sync-webhooks.md), [Mapped Types](mapped-types.md) | [Examples](../guide/examples.md#configuration-examples) |
...
>
> Use either standard query parameters (`?limit=…&orderby=…`) **or** `~` operators (`~take(…)~orderBy(…)`) in a single request. Do not combine them.
...
Query parameters are applied in a fixed internal order (not the order they appear in the URL). Use them for simple, standard filtering/sorting/paging.
...
- **Unknown Accept header parameters:** Unknown parameters in the Accept header (e.g., `Accept: application/json; unknownParam=foo`) may cause errors. Use only the documented parameters: `skipNulls`, `skipTypes`, and `numberHandling`.
