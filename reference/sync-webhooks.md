...
| `"Error while executing: The WebHook has no authorized scopes"` | Scopes became empty after creation | Re-add scopes |
| `"Error while executing: Request to {url} with item N in sequence failed with: 401"` | Authentication failure | Check `out.auth` credentials |
| `"Error while executing: Request to {url} with item N in sequence failed with: 403"` | Insufficient permissions | Verify external API permissions |
